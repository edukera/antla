...
export default class LexerAdaptor extends Lexer
{
	private PREQUEL_CONSTRUCT : number
	private OPTIONS_CONSTRUCT : number
	private _currentRuleType : number
	private insideOptionsBlock : boolean
	private _modeStack : any[] = []
	private _mode : number = 0
	private INVALID_TYPE : number = -1
	constructor(input : CharStream)
	{
		 ...
		 */
		this._currentRuleType = this.INVALID_TYPE;
	...
	emit()
	{
		...
		} else if (this._type == ANTLRv4Lexer.AT && this.getCurrentRuleType() == this.INVALID_TYPE) { // enter action
